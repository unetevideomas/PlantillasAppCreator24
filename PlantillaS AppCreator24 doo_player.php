    ...
    public static function viewer($post, $type, $players, $trailer, $size, $views, $ads = false, $image = false, $movie_title = "", $imdb_tmdb_id = "", $tv = false, $season = 0, $episode = 0){
        ...
        if(1==1){
			...
			if($ajax_player == true){
                ...
                $html .="<div class='pframe'><if"."rame class='metaframe rptss' src='{$mpembed_link}' frameborder='0' scrolling='no' allow='autoplay; encrypted-media' allowfullscreen></ifr"."ame></div>";
                $html .="</div>";
			...
			global $wpdb;
            $mpembed_get_id = $wpdb->get_results("SELECT meta_value FROM wp_postmeta WHERE post_id = '$post' AND meta_key = 'ids'");
            $mpembed_id =  $mpembed_get_id[0]->meta_value;
            if($tv) {
                //Series embed.megaplayhd.ga
                $mpembed_link = 'https://embed.megaplayhd.ga/embed/'.$mpembed_id.'/'.$season.'/'.$episode;
            } else {
                //Peliculas Embed.megaplayhd.ga
                $mpembed_link = 'https://embed.megaplayhd.ga/embed/'.$mpembed_id;
            }
			
            if($tv) {
                $html .='<li id="player-option-1" class="dooplay_player_option vs" data-post="vs" data-type='.$type.' data-nume="vs" data-vs="'.$mpembed_link.'">
                            <i class="fas fa-play-circle"></i>
                            <span class="title">Ver '.$movie_title.' </span>
                            <span class="server">FHD | Embed.megaplayhd.ga</span>
                            <span class="flag"><img src="'.DOO_URI.'/assets/img/flags/mx.png"></span>
                            <span class="loader"></span>
                        </li>';
			} else {
				  $html .='<li id="player-option-'.$num.'" class="dooplay_player_option vs" data-post="vs" data-type='.$type.' data-nume="vs" data-vs="'.$mpembed_link.'">
                            <i class="fas fa-play-circle"></i>
                            <span class="title">Ver '.$movie_title.' </span>
                            <span class="server">FHD | Embed.megaplayhd.ga</span>
                            <span class="flag"><img src="'.DOO_URI.'/assets/img/flags/mx.png"></span>
            ...
            if($trailer != false){
                ...
                $html .="<span class='title'>".__d('Ver trailer')."</span>";
				if($source_name == true)
